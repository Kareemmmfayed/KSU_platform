...
export class AdminModel {
  ...
  async authenticateAdmin(email: string, password: string): Promise<Admin> {
    ...
    const isMatched = await compareHashedPassword(password, hashedPassword);
  ...
  async indexAdmin(
    ...
    if (page && limit) {
      ...
      try {
        ...
        const query = `SELECT * FROM ${this.tableName} WHERE collage_id = $3 LIMIT $1 OFFSET $2`;
        res = await connection.query(query, [limit, offset, collageId]);
      } catch (err) {
  ...
  async updateAdmin(
    ...
    let res;
    let query;
    let queryParameters = [name, email, id, collageId];

    if (passwordInput) {
      const hashedPassword = await hashingPassword(passwordInput);
      query = `UPDATE ${this.tableName} SET name = COALESCE($1, name), password = $2, email = COALESCE($3, email) WHERE id=$4 AND collage_id=$5 RETURNING *`;
      queryParameters.splice(1, 0, hashedPassword);
    } else {
      query = `UPDATE ${this.tableName} SET name = COALESCE($1, name), email = COALESCE($2, email) WHERE id=$3 AND collage_id=$4 RETURNING *`;
    }
