...
export class ApplicationModel {
  ...
  async indexApplicationsByApplicantId(
    ...
    if (page && limit) {
      ...
      try {
        const sql = `SELECT a.*, p.name FROM application a LEFT JOIN program p ON p.id = $1 WHERE applicant_id = $2 OFFSET $3 LIMIT $4`;
      ...
      try {
        const sql = `SELECT a.*, p.name FROM application a LEFT JOIN program p ON p.id = $1 WHERE applicant_id = $2`;
        result = await connection.query(sql, [program_id, applicant_id]);
      } catch (err) {
        const msg = `Could not index applications: ${
          (err as HttpError).message
        }`;
  ...
  async indexApplicationsByProgramId(
    ...
    if (page && limit) {
      ...
      try {
        const sql = `SELECT a.*, a2.name FROM application a JOIN applicant a2 ON a2.id = a.applicant_id WHERE program_id = $1 OFFSET $2 LIMIT $3`;
      ...
      try {
        const sql = `SELECT a.*, a2.name FROM application a JOIN applicant a2 ON a2.id = a.applicant_id WHERE program_id = $1`;
