...
export class ApplicantModel {
  ...
  // Show applicant by national id
  async showByNationalId(national_id: string): Promise<any> {
    let connection;
    ...
    let res;
    try {
      const query = `SELECT * FROM applicant WHERE national_id = $1`;
      res = await connection.query(query, [national_id]);
    } catch (err) {
      const msg = `Could not get applicant to pay: ${
        (err as HttpError).message
      }`;
      const statusCode = 500;
      throw new HttpError(msg, statusCode);
    ...
    if (res.rows.length === 0) {
      const msg = `Applicant does not exist`;
      const statusCode = 500;
      throw new HttpError(msg, statusCode);
    }
