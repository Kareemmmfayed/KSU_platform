...
const indexAdmin = async (req: CollageREQ, res: Response, next: NextFunction) => {
  // page and limit
  ...
  if (!collageId)
    throw new HttpError("Invalid collage id", 400);

  let admins;
  try {
    admins = await admin.indexAdmin(page, limit, collageId);
  } catch (err) {
    return next(err);
  }
